...
usage="vfvs_pp_firstposes_prepare_vina_v9b.sh <filter summary file> <ranking structures folder> <output folder> <number of compounds>

As v9 but without replicas. Useful for v8 VS postprocessing vs v9 -> no replica folders."
...
while read -r line; do
    ...
    cp ${structures_folder}/${tranch}/${collection_no}/${molecule}/${molecule}.result.pdbqt ${vina_output_file}
    ...
    obabel -p 7.4 -ipdbqt ${output_folder}/pdbqt/first-poses/${collection}_${molecule}.pdbqt -opdb -O ${output_folder}/pdb_h/first-poses/${collection}_${molecule}.pdb || true
    counter=$((counter+1))
done < ${ranking_file} 
