...
These files need to be in text-format (space separated), with the following columns:
    * column 1: Collection
...
cd docking_scores
for file in *all; do
    awk '{print $2, $3}' $file | grep -e "-[0-9]*.[0-9]*" > $file.scores
done
...
awk '{print $1, $2}' *all | sort | uniq > compounds.all.collections+compoundids.unique
awk '{print $1","$2}' *all | sort -k 2,2 -t',' -u > compounds.all.collections_full+compoundids.unique.csv
sed -i  "1s/^/Collection,Compound_ID\n/g" compounds.all.collections_full+compoundids.unique.csv
cd ..


# new smiles way
mkdir smiles-original
cd smiles-original
...
if [ "${database_type}" == "ZINC15" ]; then
    ...
    grep -i "doctype" ../../vendors/split_${splitting_size}/* | awk -F ":" '{print $1}' | uniq | xargs rm -v ; wc -l ../../vendors/split_${splitting_size}/* | grep " 0 " | awk '{print $2}' | xargs rm -v
